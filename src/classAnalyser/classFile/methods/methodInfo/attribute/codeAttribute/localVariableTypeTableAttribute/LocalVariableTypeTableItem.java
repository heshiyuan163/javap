package classAnalyser.classFile.methods.methodInfo.attribute.codeAttribute.localVariableTypeTableAttribute;
...
public class LocalVariableTypeTableItem {
	...
	/*
	 * 所有给定的局部变量的索引都在范围[start_pc, start_pc+length)中，即从start_pc（包括自身）至start_pc+length（不包括自身）。
	...
	private byte b7;//high byte of signature_index
	private byte b8;//low byte of signature_index
	
	/*
	 * signature_index项的值必须是对常量池的一个有效索引。
	 * 常量池在该索引处的成员必须是CONSTANT_Utf8_info（§4.4.7）结构，表示给源程序中局部变量类型的字段签名（§4.3.4）。
	 */
